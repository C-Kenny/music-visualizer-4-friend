...
class CircuitMazeScene implements IScene {
  ...
  void updateLetterGrid(String text) {
    ...
    // Use a tiny blocky font feel by centering carefully
    pg.textSize(rows * 0.85); 
    pg.text(text, cols/2.0, rows/2.0 - 1);
    pg.loadPixels();
    for (int x = 0; x < cols; x++) {
      for (int y = 0; y < rows; y++) {
        letterMask[x][y] = (brightness(pg.pixels[y * cols + x]) > 128);
      }
    }
    pg.endDraw();
  }
    ...
    for (int x = 0; x < cols; x++) {
      for (int y = 0; y < rows; y++) {
        ...
        if (x % 12 == 0) { // Battery / Booster
          pg.fill(90, 255, 255, 220); pg.ellipse(px, py, 14, 14);
          pg.fill(40, 255, 255, 180 + beatFlash * 70); pg.ellipse(px, py, 8 + beatFlash * 8, 8 + beatFlash * 8);
        }
        if (x == cols - 1 && y == rows - 1) {
          float lamp = constrain(lampGlow * 255.0, 0, 255);
          pg.fill(70, 255, 220, 80 + lamp); pg.ellipse(px, py, 32 + lampGlow * 12.0, 32 + lampGlow * 12.0);
          pg.fill(120, 255, 240, 130 + lamp * 0.45); pg.ellipse(px, py, 15 + lampGlow * 5.0, 15 + lampGlow * 5.0);
        }
        if (energised) {
          if (isLetterNode(x, y)) pg.fill(50, 255, 255, 220);
          else pg.fill(90, 255, 240, 200);
        } else { pg.fill(55, 80, 90, 180); }
  ...
  void drawGate(PGraphics pg, CircuitGate g, float originX, float originY, float stepX, float stepY) {
    ...
    pg.strokeWeight(max(1.5, (2.4 + g.glow * 1.7) * (12.0/cols)));
    if (isLetterGate(g) && g.glow > 0.3) pg.stroke(50, 255, bright, alpha);
    else pg.stroke(90, 255, bright, alpha);
    float gap = 12.0 * (1.0 - g.openVis) * (12.0/cols);
    if (abs(x2 - x1) > abs(y2 - y1)) {
      pg.line(x1, y1, cx - gap, cy); pg.line(cx + gap, cy, x2, y2);
      if (g.glow > 0.5) { float p = (frameCount * 0.1) % 1.0; float sx = lerp(x1, x2, p); pg.strokeWeight(4); pg.stroke(0, 0, 255, 200 * g.glow); pg.line(sx - 5, y1, sx + 5, y1); }
      if (gap > 1.0) { pg.noStroke(); pg.fill(255, 90, 90, 190); pg.rectMode(CENTER); pg.rect(cx, cy, 4 * (12.0/cols), 11 * (12.0/cols)); }
    } else {
      pg.line(x1, y1, cx, cy - gap); pg.line(cx, cy + gap, x2, y2);
      if (g.glow > 0.5) { float p = (frameCount * 0.1) % 1.0; float sy = lerp(y1, y2, p); pg.strokeWeight(4); pg.stroke(0, 0, 255, 200 * g.glow); pg.line(x1, sy - 5, x1, sy + 5); }
      if (gap > 1.0) { pg.noStroke(); pg.fill(255, 90, 90, 190); pg.rectMode(CENTER); pg.rect(cx, cy, 11 * (12.0/cols), 4 * (12.0/cols)); }
    }
  }
