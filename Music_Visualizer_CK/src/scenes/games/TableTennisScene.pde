...
class TableTennisScene implements IScene {
  ...
  void handleKey(char k) {
    ...
    else if (k == 'r' || k == 'R') resetMatchState();
  }

  // Operator hotkey: wipe scoreboard between sets without restarting the
  // sketch. Override in subclasses that hold extra match state (e.g.
  // TableTennis3D's out-of-bounds tracking).
