...
let mandalaGraphicsBuffer;
let sceneSquareSize = 0;
let sceneSquareOffsetX = 0;
...
function setup() {
  ...
  sceneSquareSize = min(width, height);
  sceneSquareOffsetX = (width - sceneSquareSize) / 2.0;

  // Create off-screen buffer for Scene 1
  mandalaGraphicsBuffer = createGraphics(sceneSquareSize, sceneSquareSize);
  mandalaGraphicsBuffer.pixelDensity(1); // keep pixel arrays manageable

  // Scene 1: inject p5 instance and init geometry
  sceneMandala.p = mandalaGraphicsBuffer;
  sceneMandala.init(sceneSquareSize);
...
function windowResized() {
  ...
  mandalaGraphicsBuffer.remove();
  mandalaGraphicsBuffer = createGraphics(sceneSquareSize, sceneSquareSize);
  mandalaGraphicsBuffer.pixelDensity(1);
  sceneMandala.p = mandalaGraphicsBuffer;
  sceneMandala.init(sceneSquareSize);
}
...
function _drawSceneMandala() {
  ...
  mandalaGraphicsBuffer.blendMode(blendModeConstant);
  sceneMandala.draw(mandalaGraphicsBuffer);

  // Blit the buffer onto the main canvas using normal blend so the dark sides show through
  blendMode(BLEND);
  image(mandalaGraphicsBuffer, sceneSquareOffsetX, 0, sceneSquareSize, sceneSquareSize);
}
...
function _drawIdlePulse() {
  ...
  for (let ringIndex = 0; ringIndex < pulseCount; ringIndex++) {
    ...
    const ringRadius =
      baseRadius + ringIndex * radiusSpread + Math.sin(timeOffset * 0.5 + phaseOffset) * 10;
...
function keyPressed() {
  ...
  if (key === 'p') {
    ...
    if (Config.DRAW_PLASMA) {
      ...
      sceneMandala.plasma = new PlasmaEffect(
        mandalaGraphicsBuffer,
        sceneSquareSize,
        sceneSquareSize,
      );
    }
...
function _shuffleSong() {
  ...
  do {
    randomIndex = Math.floor(Math.random() * _songQueue.length);
...
function _handleControllerInput() {
  ...
  if (c.dpad_hat_switch_up) {
    Config.DRAW_TUNNEL = !Config.DRAW_TUNNEL;
    if (Config.DRAW_TUNNEL) _enableOneBg('tunnel');
  }
  if (c.dpad_hat_switch_left) {
    Config.DRAW_PLASMA = !Config.DRAW_PLASMA;
    if (Config.DRAW_PLASMA) {
      _enableOneBg('plasma');
      sceneMandala.plasma = new PlasmaEffect(
  ...
  if (c.dpad_hat_switch_right) {
    Config.DRAW_POLAR_PLASMA = !Config.DRAW_POLAR_PLASMA;
    if (Config.DRAW_POLAR_PLASMA) _enableOneBg('polar_plasma');
  }
  if (c.dpad_hat_switch_down) {
    Config.DRAW_TUNNEL = false;
    Config.DRAW_PLASMA = false;
    Config.DRAW_POLAR_PLASMA = false;
  }
...
function _setupFilePicker() {
  ...
  if (isFirefoxBrowser) {
    ...
    if (systemNoteEl) {
      systemNoteEl.innerHTML =
        "Firefox detected — System audio isn't available.<br>" +
  ...
  dropZoneEl.addEventListener('drop', (dropEvent) => {
    ...
    const droppedAudioFiles = Array.from(dropEvent.dataTransfer.files).filter((droppedFile) =>
      /\.(mp3|wav|flac|ogg|aac|m4a)$/i.test(droppedFile.name),
    );
  ...
  async function showDevicePickerForMic() {
    ...
    try {
      ...
      const labelUnlockStream = await navigator.mediaDevices.getUserMedia({
        audio: true,
        video: false,
      });
    ...
    if (audioInputDevices.length === 0) {
      ...
      audioInputDevices.forEach((audioDevice) => {
        ...
        deviceOption.textContent =
          audioDevice.label || `Microphone (${audioDevice.deviceId.slice(0, 8)}…)`;
